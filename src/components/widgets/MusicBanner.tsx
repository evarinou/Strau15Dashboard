    ...
    <div className="flex items-end gap-0.5 h-3">
      {[0, 1, 2, 3].map((i) => (
        <div
          key={i}
          className={clsx(
            'w-0.5 bg-accent rounded-full transition-all',
          ...
          style={{
            ...
            boxShadow: isPlaying
              ? '0 0 4px rgb(from var(--color-accent) r g b / 0.6)'
              : 'none',
          }}
        />
      ))}
...
export function MusicBanner({ entityId }: MusicBannerProps) {
  ...
  return (
    <div className="animate-slide-in-top mb-6">
      <div
        className={clsx(
          'glass-panel glass-l4 glass-blur-lg relative overflow-hidden',
          isPlaying ? 'ring-1 ring-accent/30' : ''
        )}
      >
        {/* Gradient background */}
        <div
          ...
          style={{
            background: isPlaying
              ? 'linear-gradient(135deg, rgb(from var(--color-accent) r g b / 0.1) 0%, transparent 50%)'
        ...
        <div className="relative z-10 p-3 sm:p-4">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <button
              ...
              style={{
                boxShadow: isPlaying
                  ? '0 0 20px rgb(from var(--color-accent) r g b / 0.4)'
                  : 'none',
              }}
            >
              {entityPicture ? (
                ...
                <Pause className="w-5 h-5 sm:w-6 sm:h-6 text-accent" />
              ) : (
              ...
              {isPlaying && (
                <div
                  ...
                  style={{
                    background: 'radial-gradient(circle, rgb(from var(--color-accent) r g b / 0.2) 0%, transparent 70%)',
                  }}
                />
              )}
            </button>
        ...
        {isPlaying && (
          <div className="absolute bottom-0 left-0 right-0 h-0.5 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-transparent via-accent/50 to-transparent animate-progress-shimmer"
              style={{
